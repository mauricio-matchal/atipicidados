...
const Step1: React.FC<{ nextStep: () => void; updateGeral: (data: Step11State) => void; updateEscola: (data: Step12State) => void }> = ({ nextStep, updateGeral, updateEscola }) => {
  ...
  return (
    <div className='flex flex-col gap-[162px] w-screen'>
      <div className='flex flex-col gap-[42px] px-5 lg:w-[840px] place-self-center'>
        <div className='flex flex-col gap-[12px]'>
          ...
          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            <FileInput placeholder='Foto 3x4' className='md:w-1/3' />
          ...
          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            ...
            <TextInput placeholder='RG' className='min-w-[260px]' value={Step11.rg} onChange={(e) => handleInputChange1("rg", e.target.value)} />
            <TextInput placeholder='CPF' className='min-w-[260px]' value={Step11.cpf} onChange={(e) => handleInputChange1("cpf", e.target.value)} />
          </div>

          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            <SelectInput className='md:w-1/3' options={["Masculino", "Feminino", "Intersexo", "Outro sexo", "Prefiro não dizer o sexo"]} placeholder={"Sexo"} onChange={(value) => handleInputChange1("sexo", value)} />
            <SelectInput className='md:w-1/3' options={["Amarelo", "Branco", "Indígena", "Pardo", "Preto"]} placeholder={"Raça/cor"} onChange={(value) => handleInputChange1("cor", value)} />
            <TextInput placeholder='CEP' className='md:w-1/3' value={Step11.cep} onChange={(e) => handleInputChange1("cep", e.target.value)} />
          </div>

          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            <TextInput className='md:w-1/2' placeholder="Endereço" value={Step11.endereco} onChange={(e) => handleInputChange1("endereco", e.target.value)} />
            <SelectInput className='md:w-1/2' options={["Salvador", "Lauro de Freitas", "Camaçari", "Catu"]} placeholder={"Cidade"} onChange={(value) => handleInputChange1("cidade", value)} />
          </div>
        </div>
        ...
        <div className='flex flex-col gap-[12px]'>
          ...
          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            <TextInput className='md:w-1/2' placeholder="Nome da escola" value={Step12.nome} onChange={(e) => handleInputChange2("nome", e.target.value)} />
            <SelectInput placeholder="Série" options={["1ª série", "2ª série", "3ª série", "4ª série", "5ª série", '6ª série', '8ª série']} className='md:w-1/2' onChange={(value) => handleInputChange2("serie", value)} />
          </div>

          <TextInput placeholder="Endereço da escola" value={Step12.endereco} onChange={(e) => handleInputChange2("endereco", e.target.value)} />

          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            <SelectInput className='md:w-1/2' placeholder={"Cidade da escola"} options={["Salvador", "Lauro de Freitas", "Camaçari", "Catu"]} onChange={(value) => handleInputChange2("cidade", value)} />
            <SelectInput className='md:w-1/2' options={["Sim, possui ADI", "Não possui ADI"]} placeholder={"Possui ADI?"} onChange={(value) => handleInputChange2("possuiAdi", value)} />
          </div>

          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            <SelectInput placeholder={"Tipo da escola"} options={["Pública", "Particular"]} className='md:w-1/2' onChange={(value) => handleInputChange2("tipo", value)} />
            <SelectInput className='md:w-1/2' placeholder={"Há quanto tempo estuda na escola?"} options={["Menos de 1 ano", "1 ano", "2 anos", "3 anos", "4 anos", "5 anos", "6 anos", "7 anos", "8 anos", "9 anos", "10 anos", "Mais de 10 anos"]} onChange={(value) => handleInputChange2("tempoNaEscola", value)} />
          </div>
          ...
          <div className='flex flex-col md:flex-row w-full gap-[12px]'>
            ...
            <FileInput
              ...
              className={`transition-opacity md:w-1/2 duration-300 ${hasRelatorio ? 'opacity-100' : 'opacity-40'} ${hasRelatorio ? '' : 'cursor-not-allowed'}`}
      ...
      {/* Rodapé */}
      <div className='relative flex flex-row justify-between items-center px-[147px]'>
        <button></button>
